...
import com.tr.cdf.aws.registry.models.Partition;
...
import com.tr.cdf.level1.client.publish.*;
 ...
 * @author Binglin Yu
 */
public class Test_relationship_CDF_1583 {
...
//    String senderId;
    ...
    String relatedObjectTypeId;

    String relationObjectTypeId;
    String s3BucketName;
...
//    String relationObjectTypeId = "ecp:9-b3c92a84-c7ec-4dd9-b37c-8bb3e618f388";
    ...
    protected void switchRegistry(String DistributionPairPartitionVersionStoreMap_EcpId,
        ...
        try {
            ...
            this.relationshipTypeId = distributionPair.getSupportedObjectTypeId();

            if (expectedAsyncEnrichment != null) {
                TestCase.assertEquals("Expected to have expected expectedAsyncEnrichment(" + expectedAsyncEnrichment
                        + ") configured in MR",
            ...
            if (expectedEnrichmentEnabled != null) {
                TestCase.assertEquals("Expected to have expected expectedEnrichmentEnabled(" + expectedEnrichmentEnabled
                        + ") configured in MR",
                        expectedEnrichmentEnabled.booleanValue(),
                        Boolean.parseBoolean(distributionPair.getEnrichmentEnableds().get(0).getValue()));
            }

            Partition partition = ecpRegistry.getById(map.getPartitionId(), Partition.class);
            System.out.println("relatedObjectType: " + partition.getRelatedObjectType());
            System.out.println("relationObjectType: " + partition.getRelationObjectType());

            this.relatedObjectTypeId = partition.getRelatedObjectType();
            this.relationObjectTypeId = partition.getRelationObjectType();
...
//        PublishRelationshipChangeEnvelopeBuilder pp = new PublishRelationshipChangeEnvelopeBuilder();
//        pp.getData()
...
//        historyBuilder.build();
        ...
        final Envelope envelope = envBuilder
                ...
                .setDeltaEmbedded(new DeltaSerialization().serialize(DeltaFactory.createDelta(Arrays.asList(change))))
    ...
    public void test_save_get() throws IOException {
        ...
        switchRegistry("ecp:9-09483403-8ff0-4dd9-baaa-e8e6281e7307", false, false);
        ...
        JSONObject jsonObject = this.save_withL2(
                relationshipId, relatedObjectId, relationObjectId,
    ...
    protected BiTemporalDataItemEntityHistory getHistory(JSONObject responseGet) {

        JSONObject jsonObject = responseGet.getJSONArray("response").getJSONObject(0)
                .getJSONObject("change").getJSONObject("newState");
    ...
    public void test_multiple_writes_enrichmentEnabled_true() {
        switchRegistry("ecp:9-a01397b4-ceab-436d-84e5-7df226becfb1", true, true);
        ...
        TestCase.assertFalse(history.getChanges().size() == 3);
